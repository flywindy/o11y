...
On the requester side, `conn.Request(ctx, subject, data, timeout)` closes the round trip. Since the otel-nats v0.6.0 upgrade the reply "receive" span is recorded by the upstream layer (not the facade): a bare `receive` span, parented under the responder's reply-send context and carrying a link to that same reply-send span when the reply carries a trace context (i.e. the responder replied via `conn.Respond`). The link does not point at the request. The reply inbox is deliberately **not** in the span name — since otel-nats v0.9.0 an inbox destination is omitted from names (it is temporary and anonymous, so it would be unbounded cardinality in every backend that keys on span name) and is queryable on `messaging.destination.name`, `messaging.message.conversation_id`, `messaging.destination.temporary` and `.anonymous`. The same rule applies to a reply sent with `conn.Publish(ctx, msg.Reply, data)` (span named `publish`) and to a handler subscribed on an inbox (`process`); see the span-name table in `docs/semconv.md`. When the reply carries no trace context (untraced responder, or one that used raw `msg.Respond`) the span is still recorded, with no link. Note the topology is two linked traces, not one: the request span lives in the requester's trace while the reply `receive` span lives in the responder's. The span that bridges them is the responder's `process` span, which links back to the requester's request span — follow *that* link in Tempo to cross between the traces; the `receive` span's own link stays inside the responder's trace. The pre-v0.6.0 variadic `attrs` parameter was removed (upstream offers no caller-attribute hook on that span); attach domain identifiers to your own ambient span instead. For a request that must carry headers, use `conn.RequestMsg(ctx, msg, timeout)`, the ctx-first shadow of the embedded ctx-less `RequestMsg`.
