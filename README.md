...
|--------|---------|-------------|
| `WithResourceAttributes(attrs ...attribute.KeyValue)` | `nil` | Add attributes to the OTel Resource next to the identity above and the detected set (`host.*`, `process.pid`, `process.executable.name`, `process.runtime.name` / `.version`, `telemetry.sdk.*`, plus `OTEL_RESOURCE_ATTRIBUTES`). They appear on `target_info` and on every span and log record, so use process-lifetime values only. Rejected with a startup warning: the four identity keys, the detected keys (`telemetry.sdk.*`, `process.pid`, `process.executable.name`, `process.runtime.name` / `.version`, `host.name`) and the rest of the `telemetry.sdk.*` and `process.*` namespaces (so `process.command_args` cannot be added back), any alias that renders as the same Prometheus label as one of those (`service_name`, `process_pid`, `host-name`), and any key that would render as an invalid label (`__meta__`). An alias of a key given earlier to the option is dropped the same way, and `OTEL_RESOURCE_ATTRIBUTES` goes through the same guard when the Resource is built, so an environment alias cannot be joined into an SDK-owned or caller-given label either. A dropped environment key never reaches `target_info` or the OTLP metrics Resource; on spans and log records it appears with an empty value (the OTel providers merge the raw environment back in, and the SDK overrides the value instead of mutating the process environment). A key set here overrides the same key from `OTEL_RESOURCE_ATTRIBUTES` |
...
| `WithHistogramBuckets([]float64)` | SLO defaults | Override the boundaries of **every** latency histogram the SDK installs a view for, not HTTP alone: `http.server.request.duration`, `http.client.request.duration`, `db.client.operation.duration` (redis, mongo, cassandra, elasticsearch), `db.client.connection.create_time` (redis, mongo, cassandra) and `minio.client.operation.duration`. A datastore's latency profile is not an HTTP service's, so buckets chosen for HTTP make those histograms coarser rather than merely different. See `DefaultLatencyBuckets()` |
| `WithDisableDefaultViews()` | off | Disable SDK-managed **HTTP** metric label allowlists and bucket views. The datastore facades' views are installed as extra views and are unaffected — they keep their label allowlists and whatever `WithHistogramBuckets` is set to |
| `WithMaxUniqueRoutes(n)` | `1000` | Cap exported distinct `http.route` values; the SDK's per-stream cardinality limit is derived from it (see `WithCardinalityLimit`) |
...
| `WithCardinalityLimit(n)` | derived: `max(2000, 4 × MaxUniqueRoutes, 4 × MaxUniqueCollections)` = `4000` | The OTel SDK's per-stream cardinality limit, applied to every instrument including the application's own: a stream holds at most `n` series, `n − 1` distinct attribute sets plus one `otel_metric_overflow="true"` series that absorbs every further set. This is the only guard against an instrument labelled by room id or user id, and the overflow series is the signal to alert on. An override below 4 is raised to 4 so the SDK's own three-series `o11y_export_failures_total` stream never overflows. Raise it only for a service whose `http.server.request.duration` legitimately carries more method × route × status combinations |
...
- [`github.com/elastic/go-elasticsearch/v8`](https://pkg.go.dev/github.com/elastic/go-elasticsearch/v8) — ships first-party OpenTelemetry tracing in its shared transport; the `elasticsearch/` facade wires the SDK `TracerProvider` into it (search body opt-in) without touching OTel globals, and records an SDK-owned `db.client.operation.duration` on the SDK `MeterProvider` since the upstream is trace-only. See [ADR 0020](docs/adr/0020-elasticsearch-integration.md) and [ADR 0027](docs/adr/0027-elasticsearch-metrics.md).
