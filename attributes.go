...
func SetUser(ctx context.Context, name string) {
	trace.SpanFromContext(ctx).SetAttributes(userbaggage.UserNameSpanAttribute(name))
}
...
func UserName(name string) slog.Attr {
	return userbaggage.UserNameLogAttr(name)
}
...
func ContextWithUser(ctx context.Context, name string) (context.Context, error) {
	return userbaggage.ContextWithUser(ctx, name)
}
