...
| `go.opentelemetry.io/contrib/instrumentation/go.mongodb.org/mongo-driver/v2/mongo/otelmongo` | v0.0.0-20260522183312-23f4e6a914b6 | ✅ | Reads globals as fallback only; never sets. Safe when `WithMeterProvider` and a noop `WithTracerProvider` are supplied for metrics-only mode. Emits semconv-compatible MongoDB operation metrics; source imports semconv v1.41.0, with the emitted keys documented in `docs/semconv.md`. | See ADR 0014 |
