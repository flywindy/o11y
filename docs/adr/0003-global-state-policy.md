...
|---|---|---|---|---|
| `akira-core/instrumentation-go/otel-nats` | v0.7.0 | ✅ | Reads provider globals as fallback only; never sets them. Safe when `WithTracerProvider` / `WithPropagators` options are supplied. The env-gated deliver-span path (its own private TracerProvider/exporter from `OTEL_EXPORTER_OTLP_ENDPOINT`) was removed upstream in v0.7.0 — the package no longer reads that env var for span emission. Module path renamed from `Marz32onE` upstream in v0.6.0. | See ADR 0004 (2026-07-16 amendment) |
