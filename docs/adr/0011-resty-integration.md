...
| `OnAfterResponse` | Set `http.response.status_code`; set span status (Error for ≥ 4xx per OTel client semconv, Unset otherwise); for status 408/504 also set `resty.error.kind=server_timeout`; if this attempt is the last one under retry policy and the response would have triggered a retry (i.e. retry budget exhausted on a retryable HTTP-error response), also set `resty.retry.exhausted=true`; record the histogram sample; end the span. |
| `OnError` | Classify the error into `resty.error.kind` (§8); set `error.type` to the underlying Go type name; set span status Error; record the histogram sample with `error.type` label; end the span. If the error is the last attempt under retry policy, additionally set `resty.retry.exhausted=true` (§8 row 7) as a separate boolean attribute so the primary `resty.error.kind` classification is preserved. |
...
| 7 | `retry_exhausted` marker | Resty's retry policy gave up: the request reached its configured retry limit and the last attempt still failed | This is **not** a `resty.error.kind` value — that key always holds the primary classification from rows 1–6 (OTel attribute keys are single-valued; reusing the key would last-value-wins overwrite the underlying classification and lose the actual cause). Instead, the **last** attempt's span gets a separate boolean attribute `resty.retry.exhausted=true` alongside its primary `resty.error.kind`. If a caller-supplied parent span exists in `req.Context()`, the same fact is recorded on that parent span as a `retry_exhausted` event via `span.AddEvent`. Detected in `OnError` by inspecting resty's request attempt count against its retry budget. |
...
| 4 | Retry exhausted on transport error | connection refused × 4 (server not listening) | `SetRetryCount(3)` with default RetryConditions plus `AddRetryCondition(func(_ *resty.Response, err error) bool { return err != nil })` | Caller → 4 sibling client spans, attempt indices 0..3; every span has `resty.error.kind=transport` and `error.type=*net.OpError` (the per-attempt classification is preserved on each span); the **last** span additionally has `resty.retry.exhausted=true` set in `OnError` after detecting `req.Attempt > RetryCount`. |
...
| 6 | Client timeout across multiple attempts | server delays response by `2 * timeout` per attempt (deterministic) | `SetTimeout(50ms)`, `SetRetryCount(3)` | Caller → ≥ 1 client span; **the last span** has `resty.error.kind=client_timeout` and `error.type=context.DeadlineExceeded`; `resty.retry.exhausted` is **not** set because the failure mode was the user-level deadline (resty's retry policy was preempted, not exhausted). Assertions are restricted to the last span and the kind attribute — span count is not asserted, since it depends on scheduler timing. |
