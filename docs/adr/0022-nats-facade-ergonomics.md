...
|---|---|---|---|
| connect | `otelnats.ConnectWithOptions` / `…WithCredentials…` | `nats.Connect(ctx,url,tp,prop,opts…)`; `nats.ConnectWithOptions(..., nats.WithTracingEnabled(sdk.Toggles.Trace), nats.WithNATSOptions(...))` supplies the SDK toggle as the local default | covered — creds pass through as `nats.UserCredentials(...)` via `Connect`, or via `WithNATSOptions` on the option-based constructor; since v0.8.0 upstream env/relay state can override that default |
